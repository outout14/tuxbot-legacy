...
class Identity(commands.Cog):
    ...
    async def ci_show(self, ctx, args: str = None):
        ...
        if user:
            self.cursor.execute(
                """SELECT userid, username, useravatar, userbirth, cidate, cibureau, os, config, pays, id FROM users WHERE userid=%s""", (str(user.id)))
            result = self.cursor.fetchone()
            ...
            if not result:
                ...
                try:
                    ...
                    formated_user_birth = str(
                        user_birth_day) + "/" + str(user_birth_month) + "/" + str(user_birth.year)
                    ...
                    try:  # a virer une fois le patch appliqué pour tout le monde
                        ...
                        cidate_month = check_date(
                            str(cidate.month))  # a garder

                        formated_cidate = str(
                            cidate_day) + "/" + str(cidate_month) + "/" + str(cidate.year)  # a garder
                    except ValueError:  # a virer une fois le patch appliqué pour tout le monde
                        # a virer une fois le patch appliqué pour tout le monde
                        formated_cidate = str(result[4]).replace('-', '/')
                        # a virer une fois le patch appliqué pour tout le monde
                        await ctx.send(f"{user.mention} vous êtes prié(e) de faire la commande `.ci update` afin de regler un probleme de date coté bdd")

                    embed = discord.Embed(
                        title="Carte d'identité | Communisme Linuxien")
                    ...
                    embed.add_field(name="Système d'exploitation :",
                                    value=isexist(result[6]), inline=True)
                    embed.add_field(name="Configuration Système : ",
                                    value=isexist(result[7]), inline=False)
                    embed.add_field(
                        name="Date de naissance sur discord : ", value=formated_user_birth, inline=True)
                    embed.add_field(name="Pays : ", value=isexist(
                        result[8]), inline=True)
                    # value=f"https://tuxbot.gnous.eu/users/{result[9]}", inline=True)
                    embed.add_field(name="Profil sur le web : ",
                                    value="*indisponible*")
                    embed.set_footer(
                        text=f"Enregistré dans le bureau {result[5]} le {formated_cidate}.")
    ...
    async def ci_register(self, ctx):
        ...
        self.cursor.execute(
            """SELECT id, userid FROM users WHERE userid=%s""", (str(ctx.author.id)))
        result = self.cursor.fetchone()
        ...
        if result:
            ...
            self.cursor.execute("""INSERT INTO users(userid, username, useravatar, userbirth, cidate, cibureau) VALUES(%s, %s, %s, %s, %s, %s)""", (str(
                ctx.author.id), str(ctx.author),  str(ctx.author.avatar_url_as(format="jpg", size=512)), str(ctx.author.created_at), now, str(ctx.message.guild.name)))
            self.conn.commit()
    ...
    async def ci_delete(self, ctx):
        ...
        if result:
            self.cursor.execute(
                """DELETE FROM users WHERE userid =%s""", (str(ctx.author.id)))
            self.conn.commit()
    ...
    async def ci_update(self, ctx):
        ...
        try:
            ...
            if result:
                self.cursor.execute(
                    """SELECT cidate FROM users WHERE userid=%s""", (str(ctx.author.id)))
                old_ci_date = self.cursor.fetchone()

                try:
                    new_ci_date = datetime.datetime.fromisoformat(
                        old_ci_date[0])
                except ValueError:
                    old_ci_date = datetime.datetime.strptime(
                        old_ci_date[0].replace('/', '-'), '%d-%m-%Y')
                    ...
                    self.cursor.execute("""UPDATE users SET cidate = %s WHERE userid = %s""", (str(
                        new_ci_date), str(ctx.author.id)))
                    self.conn.commit()

                self.cursor.execute("""UPDATE users SET useravatar = %s, username = %s, cibureau = %s WHERE userid = %s""", (str(
                    ctx.author.avatar_url_as(format="jpg", size=512)), str(ctx.author), str(ctx.message.guild), str(ctx.author.id)))
                self.conn.commit()
    ...
    async def ci_setconfig(self, ctx, *, conf: str = None):
        ...
        if conf:
            ...
            if result:
                self.cursor.execute("""UPDATE users SET config = %s WHERE userid = %s""", (str(
                    conf), str(ctx.author.id)))
                self.conn.commit()
    ...
    async def ci_setos(self, ctx, *, conf: str = None):
        ...
        if conf:
            ...
            if result:
                self.cursor.execute(
                    """UPDATE users SET os = %s WHERE userid = %s""", (str(conf), str(ctx.author.id)))
                self.conn.commit()
                await ctx.send(f"{ctx.author.mention}> :ok_hand: Carte d'identité mise à jour !")
            else:
                await ctx.send(f"{ctx.author.mention}> :x: Veuillez enregistrer votre carte d'identité pour commencer !")
    ...
    async def ci_setcountry(self, ctx, *, country: str = None):
        ...
        if country:
            ...
            if result:
                self.cursor.execute("""UPDATE users SET pays = %s WHERE userid = %s""", (str(
                    country), str(ctx.author.id)))
                self.conn.commit()
                await ctx.send(f"{ctx.author.mention}> :ok_hand: Carte d'identité mise à jour !")
            else:
                await ctx.send(f"{ctx.author.mention}> :x: Veuillez enregistrer votre carte d'identité pour commencer !")
    ...
    async def ci_online_edit(self, ctx):
        ...
        self.cursor.execute(
            """SELECT id FROM users WHERE userid=%s""", (str(ctx.author.id)))
        result = self.cursor.fetchone()
        ...
        try:
            def is_exist(key, value):
                self.cursor.execute(
                    """SELECT * FROM sessions WHERE {}=%s""".format(str(key)), (str(value)))
                return self.cursor.fetchone()

            user_id = result[0]
            is_admin = '1' if str(
                ctx.author.id) in self.bot.config.authorized_id else '0'
            token = ''.join(random.sample(
                'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'*25, 25))
            ...
            if is_exist('user_id', user_id):
                self.cursor.execute("""UPDATE sessions SET is_admin = %s, token = %s, updated_at = %s WHERE user_id = %s""", (str(
                    is_admin), str(token), str(created_at), str(user_id)))
                self.conn.commit()
            else:
                self.cursor.execute("""INSERT INTO sessions(user_id, is_admin, token, created_at, updated_at) VALUES(%s, %s, %s, %s, %s)""", (str(
                    user_id), str(is_admin),  str(token), str(created_at), str(created_at)))
                self.conn.commit()
